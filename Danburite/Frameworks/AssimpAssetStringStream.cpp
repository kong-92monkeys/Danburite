...
namespace Frx
{
	AssimpAssetStringStream::AssimpAssetStringStream(
		std::string_view const &assetPath)
	{
		auto &assetManager{ Sys::Env::getInstance().getAssetManager() };
		__asset = assetManager.readString(assetPath);
	}

	size_t AssimpAssetStringStream::Read(
		void *const pvBuffer,
		size_t const pSize,
		size_t const pCount)
	{
	...
	size_t AssimpAssetStringStream::Write(
	...
	aiReturn AssimpAssetStringStream::Seek(
	...
	size_t AssimpAssetStringStream::Tell() const
	{
		return __cursor;
	}

	size_t AssimpAssetStringStream::FileSize() const
	{
		return __asset.size();
	}

	void AssimpAssetStringStream::Flush()
	{}
}