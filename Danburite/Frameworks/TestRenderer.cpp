...
namespace Frx
{
	...
	std::unique_ptr<VK::Framebuffer> TestRenderer::createFramebuffer(
	...
	std::unique_ptr<VK::Pipeline> TestRenderer::createPipeline(
		VK::RenderPass &renderPass,
		uint32_t const outputWidth,
		uint32_t const outputHeight) const
	{
	...
	Render::Renderer::InitResult TestRenderer::_onInit()
	{
		__pVertexShader		= _createShaderModule("Shaders/Test.vert");
		__pFragmentShader	= _createShaderModule("Shaders/Test.frag");
	...
	std::shared_ptr<VK::PipelineLayout> TestRenderer::__createPipelineLayout() const
	{
